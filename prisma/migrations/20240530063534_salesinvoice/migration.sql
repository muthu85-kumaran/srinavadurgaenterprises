/*
  ...
  - A unique constraint covering the columns `[customerId]` on the table `SalesInvoice` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `customerId` to the `SalesInvoice` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `salesinvoice` ADD COLUMN `customerId` VARCHAR(191) NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX `SalesInvoice_customerId_key` ON `SalesInvoice`(`customerId`);
