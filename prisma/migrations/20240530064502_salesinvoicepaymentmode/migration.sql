/*
  ...
  - A unique constraint covering the columns `[paymentModeId]` on the table `SalesInvoice` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `paymentModeId` to the `SalesInvoice` table without a default value. This is not possible if the table is not empty.
...
-- DropIndex
DROP INDEX `Receipt_vendorId_fkey` ON `receipt`;
...
-- AlterTable
ALTER TABLE `salesinvoice` ADD COLUMN `paymentModeId` VARCHAR(191) NOT NULL;

-- CreateIndex
CREATE UNIQUE INDEX `SalesInvoice_paymentModeId_key` ON `SalesInvoice`(`paymentModeId`);
