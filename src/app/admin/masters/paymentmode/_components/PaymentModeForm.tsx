...
import { SavePaymentModeAction } from "@/actions/masters/PaymentModeAction";
...
const PaymentModeForm = ({ PaymentMode }: PaymentModeFormProps) => {
  ...
  const zodform = useForm<z.infer<typeof PaymentModeSchema>>({
    resolver: zodResolver(PaymentModeSchema),
    defaultValues: {
      name: PaymentMode?.name || "",
      description: PaymentMode?.description || "",
      id: PaymentMode?.id || undefined,
    },
    mode: "onBlur",
  });
  const { control, handleSubmit, reset, formState } = zodform;
  const { isSubmitting } = formState;
  const processForm: SubmitHandler<z.infer<typeof PaymentModeSchema>> = async (
    data
  ) => {
    ...
    const result = await SavePaymentModeAction(newPaymentMode);
    if (!result) {
    ...
    if (result.success) {
      ...
      router.push("/admin/masters/paymentmode");
    }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Mode Of Payment" {...field} />
                  </FormControl>
