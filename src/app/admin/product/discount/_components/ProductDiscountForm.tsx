...
import { SaveProductDiscountAction } from "@/actions/products/ProductDiscountAction";
...
const ProductDiscountForm = ({ ProductDiscount }: ProductDiscountFormProps) => {
  ...
  const formData = useForm<z.infer<typeof ProductDiscountSchema>>({
    resolver: zodResolver(ProductDiscountSchema),
    defaultValues: {
      discountInPercent: ProductDiscount?.discountInPercent || 0,
      discountInAmount: ProductDiscount?.discountInAmount || 0,
      discountStartDate: ProductDiscount?.discountStartDate || undefined,
      discountEndDate: ProductDiscount?.discountEndDate || undefined,
      id: ProductDiscount?.id || undefined,
    },
  });

  const formAction = async (formData: FormData) => {
    const newProductDiscount = {
      discountInPercent: formData.get("discountInPercent"),
      discountInAmount: formData.get("discountInAmount"),
    ...
    //post data to server
    const result = await SaveProductDiscountAction(validation.data);
        ...
        >
          <div className="grid grid-cols-1 gap-y-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel> Discount In Amount </FormLabel>
                  <FormControl>
                    <Input placeholder="" {...field} />
                  </FormControl>
          ...
          <div className="flex justify-center">
            ...
            <Button
              ...
              onClick={() => {
                ...
                router.push("/admin/product/discount/add");
              }}
            >
              Close
            </Button>
