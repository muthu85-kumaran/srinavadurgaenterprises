...
import ListProductDiscount from "./_components/ListProductDiscount";
...
const ProductDiscountPage = async () => {
  const response = await listProductDiscountAction();
  if (!response.success) {
  ...
  return (
    <>
      ...
      <div className="flex justify-between bg-cyan-100 px-8 pt-4 pb-2 rounded-md shadow-md">
        <Link href="/admin/productdiscount/add">
          <div className="flex items-center cursor-pointer">
            <PlusSquareIcon className="w-8 h-8 text-blue-500" />
            <span className="ml-2">Add Product Discount </span>
          </div>
        </Link>
      </div>
      <div className="mt-8">
        <Suspense fallback={<Loading />}>
          <ListProductDiscount tableData={response.data} />
        </Suspense>
      </div>
    </>
  );
