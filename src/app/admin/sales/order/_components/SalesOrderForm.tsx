...
import { SaveSalesOrderAction } from "@/actions/sales/SalesOrderAction";
import {
  cn,
  getErrorMessage,
...
const SalesOrderForm = ({
  ...
  const zodForm = useForm<z.infer<typeof SalesOrderSchema>>({
  ...
  const processForm: SubmitHandler<z.infer<typeof SalesOrderSchema>> = async (
    data
  ) => {
    console.log(data);
    try {
      const result = await SaveSalesOrderAction(data);
      if (!result) {
      ...
      <Form {...zodForm}>
        <form className="space-y-2 mb-4" onSubmit={handleSubmit(processForm)}>
          <div className="bg-lime-300 p-3">
            <p className="text-stone-800">Sales Order </p>
          </div>
          ...
          <div className="flex justify-center">
            <Button
              ...
              // onClick={(e) => {
              //   e.preventDefault();
              //   console.log(SalesOrderSchema.safeParse(zodForm.getValues()));
              // }}
            >
              {isSubmitting ? (
