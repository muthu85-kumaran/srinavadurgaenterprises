...
import { listSalesOrderAction } from "@/actions/sales/SalesOrderAction";
...
const SalesOrderPage = async () => {
  const response = await listSalesOrderAction();
  if (!response) {
  ...
  return (
    <>
      ...
      <div className="flex justify-between bg-cyan-100 px-8 pt-4 pb-2 rounded-md shadow-md">
        <Link href="/admin/sales/order/add">
          <div className="flex items-center cursor-pointer">
            <PlusSquareIcon className="w-8 h-8 text-blue-500" />
            <span className="ml-2">New Sales Order </span>
          </div>
        </Link>
      </div>
      <div className="mt-8">
        <Suspense fallback={<Loading />}>
          <ListDataTable<SalesOrder>
            tableData={response.data as SalesOrder[]}
            columns={columns}
